        ...
        def complete_e(words, postags, child_dict_list, word_index):
            if expand == "all" or (expand == "exclude_entity" and "#"+postags[word_index]+"#" not in self.entity_types):
                ...
                if postags[word_index].lower().startswith('v'):
                    ...
                    # 并列动词合并
                    if '并列关系' in child_dict and postags[child_dict['并列关系'][0]].startswith('v'):
                        postfix += complete_e(words, postags, child_dict_list, child_dict['并列关系'][0])
                    ...
                    for m in range(len(child_dict['主谓关系'])):
                        ...
                        if '并列关系' in child_dict and postags[index].lower().startswith('v'):
                            ...
                            for i, para_c in enumerate(sorted(child_dict['并列关系'])):
                                if postags[para_c].lower().startswith('v') and para_c-index == i+1:
                                    r += words[para_c]
                                else:
